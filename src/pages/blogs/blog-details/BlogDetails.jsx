...
const BlogDetails = ({ id }) => {
  ...
  const handleCopy = () => {
    setCopied(blogDetails?.experience);
    navigator.clipboard.writeText(blogDetails?.experience);
    setTimeout(() => {
      setCopied("");
    }, 1000);
  };
          ...
          <div className="max-w-lg p-0.5 bg-gradient-to-tr from-yellow-200 via-slate-400 to-pink-200 rounded-lg max-h-[368px]">
            <div className="max-w-lg max-h-[368px] border relative bg-base-200 shadow-md p-6 overflow-hidden rounded-lg min-h-full">
              ...
              <article>
                <button
                  onClick={handleCopy}
                  className=" absolute  top-7 right-1.5  flex  font-medium"
                >
                  {copied == blogDetails?.experience ? (
