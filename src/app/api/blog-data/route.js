...
export const POST = async (request) => {
  ...
  try {
    const db = await connectDB();
    const blogsCollection = await db.collection("blogs");
    const findBlog = await blogsCollection.findOne({
      title: blogFromClient?.title,
    });
    if (findBlog) {
...
export const PUT = async (request) => {
  ...
  try {
    ...
    const updateReactsOfBlog = await blogsCollection.updateOne(
      { _id: new ObjectId(id) },
      {
        $addToSet: { reactedBy: email }, // Add the email to reactedBy
        $inc: { react: 1 },             // Increment react count by 1
      },
      { upsert: true }
    );
...
export const DELETE = async (request) => {
  ...
  try {
    const db = await connectDB();
    const blogsCollection = await db.collection("blogs");
    const deleteBlog = await blogsCollection.deleteOne({
      _id: new ObjectId(id),
    });
    // console.log(BlogsData)
...
const data = [
  ...
  },
  {
    ...
    "reactedBy": ["redoyhasan4585@gmail.com"]
  },
  {
    "user": "Daniel Kim",
